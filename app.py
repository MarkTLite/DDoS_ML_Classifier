...
def main():
    ...
    if st.button('DDoS Test Result'):
        network = DDoS_Prediction(
          [float(pktcount),
           float(bytecount),
            float(dur), float(dur_nsec),
            float(tot_dur), float(flows), float(pktperflow), float(byteperflow),
             float(pktrate), float(port_no), float(tx_bytes),
       float(rx_bytes), float(tx_kbps), float(randint(0,1)), float(randint(0,1)), float(randint(0,1)), float(randint(0,1))])
